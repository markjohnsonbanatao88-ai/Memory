...
export function createOperatorMemoryQaFlowRouteHandler(deps: OperatorMemoryQaFlowRouteDependencies = {}) { return async function POST(request: NextRequest) {
  ...
  const rejected = await assertNoClientUserIdOverride(request, body); if (rejected) return NextResponse.json({ ...disabled, blockers: ["client_user_id_rejected"] }, { status: 400 });
  const runtime = resolvePandoraRuntimeSafetyConfig(deps.env?.() ?? process.env);
  if (!deps.enabled) return NextResponse.json({ ...disabled, gates: runtime.gates }, { status: 501 });
